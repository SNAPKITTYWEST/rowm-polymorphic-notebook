  ...
  <template id="cellTemplate">
    <article class="iso-cell">
      ...
      <textarea
        class="cell-source"
        spellcheck="false"
        aria-label="Unicode notebook cell"
      ></textarea>
...
<script type="module">
  class IsomorphicUnicodeEngine {
    ...
    fromUnicodeIR(ir) {
      if (!ir || ir.version !== "unicode-ir/1") {
        throw new Error("Unsupported Unicode IR");
      }

      const reconstructed = ir.codePoints
        .map((entry) => {
          if (!/^U\+[0-9A-F]{4,6}$/u.test(entry)) {
            throw new Error(`Invalid code point: ${entry}`);
          }
      ...
      if (reconstructed !== decodedUtf8) {
        throw new Error("Unicode IR representations disagree");
      }
    ...
    async reverseCell(cell) {
      ...
      try {
        ...
        if (restored !== expected) {
          throw new Error("Round-trip invariant failed");
        }
    ...
    async sha512(value) {
      if (!globalThis.crypto?.subtle) {
        throw new Error("Web Crypto SHA-512 unavailable");
      }
